...
data "aws_iam_policy_document" "images_cdn" {
  statement {
    ...
    resources = ["arn:aws:s3:::ns08014buket/*"]
...
resource "aws_cloudfront_origin_access_identity" "images_cdn" {
  comment = "ns08014_origin_access_identity"
}
...
resource "aws_s3_bucket" "log_bucket" {
  bucket        = "ns08014logbuket"
  force_destroy = true
  tags = {
    Name = "ns08014logbuket"
  }
}
...
resource "aws_cloudfront_distribution" "images_cdn" {
  ...
  logging_config {
    ...
    bucket = "ns08014logbuket.s3.amazonaws.com"
    prefix = "ns08014"
  }

  comment = "ns08014_images_cdn"
  
  enabled             = true
  is_ipv6_enabled     = true
  wait_for_deployment = false #cloudfront status가 deployed전 먼저 terraform apply completed
  default_root_object = "ns08014key"
  ...
  tags = {
    Name = format("%s-cloudfront", "ns08014")
  }
}
